...
namespace AutoVariable
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            var myVariable = KeyAuthApp.webhook("webhookIDForGetVar", "user=" + KeyAuthApp.user_data.username + "&var=VariableNameToRetrieve");                 // the string after var can be anything you'd like, 
            ...
            {                                                                                                                                                   // 
                KeyAuthApp.webhook("webhookIDForSetVar", "user=" + KeyAuthApp.user_data.username + "&var=VariableNameToSet&data=VariableDataYouWantToSet");     // If the variable does not exist then we will attemp to create a new one... note this webhook is different than the one on line 46. This one creates a new var, the other one gathers var data
                ...
                var newlyCreatedVar = KeyAuthApp.webhook("webhookIDForGetVar", "user=" + KeyAuthApp.user_data.username + "&var=VariableNameToRetrieve");        // we will then create another var to get the new response from the new request to create a variable, for this you want to put the webhookID for the RETRIEVEVAR (on line 46)
